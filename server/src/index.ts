...
const app = express()
app.disable('x-powered-by'); app.use(helmet({ crossOriginResourcePolicy: false })); app.use(cors({ origin: env.FRONTEND_ORIGIN, methods: ['GET','POST','PUT','PATCH','DELETE'], allowedHeaders: ['Content-Type','Authorization'] })); app.use(express.json({ limit: '1mb' }))
app.get('/health', (_req, res) => res.json({ ok: true }))
app.use('/internal', internal)
app.use('/api', (req, res, next) => ['/auth/login', '/collectors/pair', '/collectors/heartbeat', '/sms'].includes(req.path) ? next() : requireAuth(req, res, next), router)
